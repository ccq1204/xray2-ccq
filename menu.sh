...
install_Xray2() {
    ...
    if  [ $# == 0 ] ;then
        ...
        wget --no-check-certificate -N --progress=bar -O /usr/local/Xray2/Xray2-linux.zip https://github.com/wyx2685/Xray2/releases/download/${last_version}/Xray2-linux-${arch}.zip
        if [[ $? -ne 0 ]]; then
        ...
        last_version=$1
        url="https://github.com/wyx2685/Xray2/releases/download/${last_version}/Xray2-linux-${arch}.zip"
        echo -e "开始安装 Xray2 $1"
